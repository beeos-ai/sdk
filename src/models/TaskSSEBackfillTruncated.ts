...
/**
 * Body of an `event: backfill_truncated` frame on
 * `/tasks/{taskId}/events` and
 * `/conversations/{convId}/events`. Forwarded verbatim from
 * Message Service (ADR-0022 §1.2 / R2): emitted when the client
 * reconnected with `since=N` where N is older than the oldest
 * entry still retained in MS's per-channel Redis Stream —
 * chunks within that window have been evicted.
 * 
 * SDKs SHOULD react by either:
 *   1. resuming from `oldest_redis_offset` (skipping the evicted
 *      chunks but staying on the live stream), or
 *   2. refetching the polling endpoint with
 *      `?include_deltas=true` for a best-effort backfill of
 *      whatever PG still has.
 * 
 * Either path keeps the consumer on a consistent offset
 * sequence; ignoring the frame is also legal — clients that
 * only care about terminal `agent_reply` envelopes lose nothing
 * because the terminal row is never evicted.
 * 
 * @export
 * @interface TaskSSEBackfillTruncated
 */
export interface TaskSSEBackfillTruncated {
    /**
     * Smallest offset still readable from the per-channel
     * Redis Stream. Clients can resume from any
     * `offset >= oldest_redis_offset` without missing chunks.
     * 
     * @type {number}
     * @memberof TaskSSEBackfillTruncated
     */
    oldestRedisOffset: number;
    /**
     * The cursor the client reconnected with (echoed back for
    ...
    since: number;
    /**
     * Human-readable English string explaining the gap.
     * Informational only — SDKs should not pattern-match on it.
     * 
     * @type {string}
     * @memberof TaskSSEBackfillTruncated
     */
...
/**
 * Check if a given object implements the TaskSSEBackfillTruncated interface.
 */
export function instanceOfTaskSSEBackfillTruncated(value: object): value is TaskSSEBackfillTruncated {
    if (!('oldestRedisOffset' in value) || value['oldestRedisOffset'] === undefined) return false;
    if (!('since' in value) || value['since'] === undefined) return false;
    return true;
}

export function TaskSSEBackfillTruncatedFromJSON(json: any): TaskSSEBackfillTruncated {
    return TaskSSEBackfillTruncatedFromJSONTyped(json, false);
}

export function TaskSSEBackfillTruncatedFromJSONTyped(json: any, ignoreDiscriminator: boolean): TaskSSEBackfillTruncated {
    if (json == null) {
        return json;
    }
    return {
        
        'oldestRedisOffset': json['oldest_redis_offset'],
        'since': json['since'],
        'hint': json['hint'] == null ? undefined : json['hint'],
    };
}

export function TaskSSEBackfillTruncatedToJSON(json: any): TaskSSEBackfillTruncated {
    return TaskSSEBackfillTruncatedToJSONTyped(json, false);
}

export function TaskSSEBackfillTruncatedToJSONTyped(value?: TaskSSEBackfillTruncated | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
    ...
    return {
        ...
        'oldest_redis_offset': value['oldestRedisOffset'],
        'since': value['since'],
        'hint': value['hint'],
    };
}
