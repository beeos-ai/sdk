...
export interface PresignUploadResponse {
    ...
    fileId: string;
    /**
     * Presigned PUT URL. Client uploads bytes directly with the same
     * `Content-Type` supplied in the request. Single-use; expires per
    ...
    uploadUrl: string;
    /**
     * Presigned GET URL for the same file, when the storage backend
     * supports it (S3 / MinIO). Empty for backends that don't presign
     * downloads — fall back to `GET /api/v1/files/{id}` in that case.
     * 
     * @type {string}
     * @memberof PresignUploadResponse
     */
...
export function PresignUploadResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): PresignUploadResponse {
    ...
    return {
        ...
        'downloadExpiresIn': json['download_expires_in'] == null ? undefined : json['download_expires_in'],
    };
}
...
export function PresignUploadResponseToJSONTyped(value?: PresignUploadResponse | null, ignoreDiscriminator: boolean = false): any {
    ...
    return {
        ...
        'download_expires_in': value['downloadExpiresIn'],
    };
}
