...
/**
 * Body of the terminal `event: end` frame. SDKs treat any value
 * as a clean shutdown signal; the `reason` field disambiguates
 * whether the agent terminated (`task_terminal`), MS closed the
 * upstream stream (`stream_closed`), or the conversation channel
 * was deleted (`channel_closed`).
 * 
 * @export
 * @interface TaskSSEEnd
 */
export interface TaskSSEEnd {
    /**
     * 
     * @type {string}
     * @memberof TaskSSEEnd
     */
    reason: TaskSSEEndReasonEnum;
}
 ...
 * @export
 */
export const TaskSSEEndReasonEnum = {
    TaskTerminal: 'task_terminal',
    StreamClosed: 'stream_closed',
    ChannelClosed: 'channel_closed'
} as const;
export type TaskSSEEndReasonEnum = typeof TaskSSEEndReasonEnum[keyof typeof TaskSSEEndReasonEnum];


/**
 * Check if a given object implements the TaskSSEEnd interface.
 */
export function instanceOfTaskSSEEnd(value: object): value is TaskSSEEnd {
    if (!('reason' in value) || value['reason'] === undefined) return false;
    return true;
}

export function TaskSSEEndFromJSON(json: any): TaskSSEEnd {
    return TaskSSEEndFromJSONTyped(json, false);
}

export function TaskSSEEndFromJSONTyped(json: any, ignoreDiscriminator: boolean): TaskSSEEnd {
    if (json == null) {
        return json;
    }
    return {
...
export function TaskSSEEndToJSON(json: any): TaskSSEEnd {
    return TaskSSEEndToJSONTyped(json, false);
}

export function TaskSSEEndToJSONTyped(value?: TaskSSEEnd | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
