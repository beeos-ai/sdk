...
/**
 * Request body for `PATCH /api/v3/conversations/{cid}/messages/{mid}`.
 * All fields are optional; the wire is SNAPSHOT-based — each
 * PATCH carries the full cumulative body + parts the server
 * should persist (NOT an indexed delta).
 * 
 * Returns `410 Gone` (`code = message_terminal`) if the row is
...
/**
 * Check if a given object implements the PatchMessageRequest interface.
 */
export function instanceOfPatchMessageRequest(value: object): value is PatchMessageRequest {
    return true;
}

export function PatchMessageRequestFromJSON(json: any): PatchMessageRequest {
    return PatchMessageRequestFromJSONTyped(json, false);
}

export function PatchMessageRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): PatchMessageRequest {
    if (json == null) {
        return json;
    }
    return {
...
export function PatchMessageRequestToJSON(json: any): PatchMessageRequest {
    return PatchMessageRequestToJSONTyped(json, false);
}

export function PatchMessageRequestToJSONTyped(value?: PatchMessageRequest | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
