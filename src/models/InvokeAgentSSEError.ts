...
/**
 * Recoverable error frame. The stream still emits a final `done`
 * with `is_error: true` after this (audit-v4 P1-1) so SDKs can
 * branch on either signal.
 * 
 * @export
 * @interface InvokeAgentSSEError
 */
export interface InvokeAgentSSEError {
    /**
     * 
     * @type {string}
     * @memberof InvokeAgentSSEError
     */
    type: InvokeAgentSSEErrorTypeEnum;
    /**
     * Stable sentinel string code. Canonical values include
     * `agent_not_found`, `agent_unavailable`, `agent_timeout`,
     * `forbidden`, `invalid_param`, `internal`.
     * 
     * @type {string}
     * @memberof InvokeAgentSSEError
     */
 ...
 * @export
 */
export const InvokeAgentSSEErrorTypeEnum = {
    Error: 'error'
} as const;
export type InvokeAgentSSEErrorTypeEnum = typeof InvokeAgentSSEErrorTypeEnum[keyof typeof InvokeAgentSSEErrorTypeEnum];


/**
 * Check if a given object implements the InvokeAgentSSEError interface.
 */
export function instanceOfInvokeAgentSSEError(value: object): value is InvokeAgentSSEError {
    if (!('type' in value) || value['type'] === undefined) return false;
    if (!('message' in value) || value['message'] === undefined) return false;
    return true;
}

export function InvokeAgentSSEErrorFromJSON(json: any): InvokeAgentSSEError {
    return InvokeAgentSSEErrorFromJSONTyped(json, false);
}

export function InvokeAgentSSEErrorFromJSONTyped(json: any, ignoreDiscriminator: boolean): InvokeAgentSSEError {
    if (json == null) {
        return json;
    }
    return {
...
export function InvokeAgentSSEErrorToJSON(json: any): InvokeAgentSSEError {
    return InvokeAgentSSEErrorToJSONTyped(json, false);
}

export function InvokeAgentSSEErrorToJSONTyped(value?: InvokeAgentSSEError | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
