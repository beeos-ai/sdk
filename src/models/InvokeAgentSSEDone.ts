...
/**
 * Terminal frame. Clients should release the SSE connection on
 * this frame. `is_error: true` may be paired with a populated
 * `error` / `code` carrying the same payload as the preceding
 * `InvokeAgentSSEError` frame so single-frame consumers can handle
 * errors without buffering history.
 * 
 * @export
 * @interface InvokeAgentSSEDone
 */
export interface InvokeAgentSSEDone {
    /**
     * 
     * @type {string}
     * @memberof InvokeAgentSSEDone
     */
    type: InvokeAgentSSEDoneTypeEnum;
    /**
    ...
    error?: string;
    /**
     * Stable sentinel code (only when `is_error=true`). Matches
     * the `code` on the preceding `InvokeAgentSSEError` frame.
     * 
     * @type {string}
     * @memberof InvokeAgentSSEDone
     */
 ...
 * @export
 */
export const InvokeAgentSSEDoneTypeEnum = {
    Done: 'done'
} as const;
export type InvokeAgentSSEDoneTypeEnum = typeof InvokeAgentSSEDoneTypeEnum[keyof typeof InvokeAgentSSEDoneTypeEnum];


/**
 * Check if a given object implements the InvokeAgentSSEDone interface.
 */
export function instanceOfInvokeAgentSSEDone(value: object): value is InvokeAgentSSEDone {
    if (!('type' in value) || value['type'] === undefined) return false;
    return true;
}

export function InvokeAgentSSEDoneFromJSON(json: any): InvokeAgentSSEDone {
    return InvokeAgentSSEDoneFromJSONTyped(json, false);
}

export function InvokeAgentSSEDoneFromJSONTyped(json: any, ignoreDiscriminator: boolean): InvokeAgentSSEDone {
    if (json == null) {
        return json;
    }
    return {
...
export function InvokeAgentSSEDoneToJSON(json: any): InvokeAgentSSEDone {
    return InvokeAgentSSEDoneToJSONTyped(json, false);
}

export function InvokeAgentSSEDoneToJSONTyped(value?: InvokeAgentSSEDone | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
